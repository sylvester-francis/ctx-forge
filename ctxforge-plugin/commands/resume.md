...
1. Parse `{{args}}` for `--memory-limit <N>`. Default `N = 5`.
...
5. If the bundle is empty AND there are no notes, say so and suggest `/ctxforge` (the guided workflow).
...
- `/ctxforge` — guided bundle-building workflow for a fresh session.
